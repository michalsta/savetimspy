...
def iter_group_based_views_of_data(
    ...
    assert (
        grouping_column_name in df.columns
    ), f"Lacking column {grouping_column_name} in the submitted 'df'."
    groups = df[grouping_column_name].values
    if assert_sorted:
        assert is_sorted(
            groups
        ), "The df needs to be sorted with respect to frames for 'iter_frame_based_views' to iterate over views of data properly without any copies."
    i_prev = 0
    group_prev = groups[0]
